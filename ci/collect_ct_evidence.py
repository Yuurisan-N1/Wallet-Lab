...
Usage:
  python3 ci/collect_ct_evidence.py
  python3 ci/collect_ct_evidence.py --build-dir build/copilot-ci
  python3 ci/collect_ct_evidence.py --output-dir /tmp/ufsecp-ct
  python3 ci/collect_ct_evidence.py --runner-binary build/audit/unified_audit_runner
  python3 ci/collect_ct_evidence.py --strict
"""
...
LAYER_SPECS = [
    ...
    LayerSpec(
        ...
        configured_surfaces=['.github/workflows/valgrind-ct.yml', 'docs/CT_VERIFICATION.md', 'ci/valgrind_ct_check.sh', 'audit/test_ct_sidechannel.cpp'],
    ...
    LayerSpec(
        ...
        configured_surfaces=['docs/CT_VERIFICATION.md', 'ci/verify_ct_disasm.sh'],
        required_for_owner_grade=True,
    ),
    LayerSpec(
...
def _run_disasm_if_missing(repo_root: Path, runner_binary: Path | None, output_dir: Path) -> list[str]:
    ...
    if json_path.exists():
        notes.append('generated disasm_branch_scan.json via ci/verify_ct_disasm.sh')
